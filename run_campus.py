...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transition_scenario", action="store", default="extend-lockdown"
    )
    parser.add_argument("--transition_mode", action="store", type=int, default="1")
    parser.add_argument(
        "--ci_multiplier",
        action="store",
        type=float,
    ...
    outfile = "{}/{}-{}-{}.csv".format(
        output_dir, location, transition_scenario, transition_day
    )
    if args.generic_outfile:
        outfile = "{}/out.csv".format(output_dir)

    end_time = 180
    if transition_scenario in [
        "extend-lockdown",
        "dynamic-lockdown",
        "periodic-lockdown",
        "uk-forecast",
    ...
    e.disease = read_disease_yml.read_disease_yml("{}/disease.yml".format(data_dir))

    building_file = "{}/{}_buildings.csv".format(data_dir, location)
    # print(building_file)

    households_per_house = (
        13  # Brunel has large residences, where many households reside. 38*8* ~ 4000
    )
    ...
    read_building_csv.read_building_csv(
        ...
        office_size=1600,
        household_size=8,
        households_per_house=households_per_house,
    ...
    for t in range(0, end_time):
        ...
        t_adjusted = (
            t + 204
        )  # Adjusted t for measures to account for a start date in September.
        ...
        if transition_scenario in ["uk-forecast"]:
            ...
            measures.uk_lockdown_existing(
                e, t_adjusted, track_trace_limit=track_trace_limit
            )
