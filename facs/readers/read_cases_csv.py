        ...
        for row in cases_reader:
            if len(row[3]) > 0:
                ...
                if day < 0 and day > -period_to_rec:
                    e.add_infection(
                        float(row[0]), float(row[1]), int(row[2]), day - period_to_hosp
                    )
                    e.add_infections(
                        16, day - period_to_hosp
                    )  # 1 hospitalisation per 16.67 infections (0.06 prob).
                    num_infections += 17
                e.add_validation_point(day)
    print(
        "Using start date {} with {} infections initially.".format(
            start_date, num_infections
        )
    )
